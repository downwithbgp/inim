#!/usr/bin/env bash
# Session 43 visual-review harness: compact findings viewports.
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
DB="${UI_REVIEW_DB:-data/inim.sqlite}"
OUT=tmp/ui-review/session43
PORT="${UI_REVIEW_PORT:-8224}"
BIND="127.0.0.1:$PORT"
mkdir -p "$OUT"
...
declare -a PAGES=(
  ...
  "uva-prepend-expanded-1280x800:/events/INC0299001/workbench?expand=1:1280,800"
  "uva-withdrawal-route-sequence-1280x800:/events/INC0299001/workbench?expand=1:1280,800"
...
echo "session43 screenshots written to $OUT/"
