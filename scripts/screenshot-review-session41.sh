#!/usr/bin/env bash
# Session 41 visual-review harness: compact findings viewports.
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
DB="${UI_REVIEW_DB:-data/inim.sqlite}"
OUT=tmp/ui-review/session41
PORT="${UI_REVIEW_PORT:-8212}"
BIND="127.0.0.1:$PORT"
mkdir -p "$OUT"
...
echo "session41 screenshots written to $OUT/"
