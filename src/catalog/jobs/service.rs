...
/// Cancel a QUEUED job because it is outside the configured project
/// scope. Distinct from analytical failure: the job was valid when
/// queued but is now intentionally excluded. Never run, zero network.
pub fn cancel_queued_excluded(conn: &Connection, job_id: &str) -> Result<(), String> {
    transition(
        conn,
        job_id,
        JobState::Queued,
        ...
        "Job is outside the configured project scope; cancelled before execution",
    )
}
...
mod tests {
    ...
    fn blocked_plan_cannot_be_queued() {
        ...
        let err = queue(&conn, plan, RequestSource::Cli, "h", &crate::catalog::scope::ProjectScope::default()).unwrap_err();
    ...
    fn ready_plan_can_be_queued() {
        ...
        let out = queue(&conn, plan, RequestSource::Cli, "hash-1", &crate::catalog::scope::ProjectScope::default()).unwrap();
        let id = match out {
    ...
    fn duplicate_submit_returns_existing_active_job() {
        let conn = test_conn();
        let plan = seed_ready_plan(&conn);
        let first = queue(&conn, plan, RequestSource::Cli, "hash-1", &crate::catalog::scope::ProjectScope::default()).unwrap();
        let second = queue(&conn, plan, RequestSource::LocalWeb, "hash-1", &crate::catalog::scope::ProjectScope::default()).unwrap();
        match (first, second) {
    ...
    fn completed_job_does_not_block_explicit_new_run() {
        ...
        let plan = seed_ready_plan(&conn);
        let first = match queue(&conn, plan, RequestSource::Cli, "hash-1", &crate::catalog::scope::ProjectScope::default()).unwrap() {
            QueueOutcome::Created(id) => id,
            _ => unreachable!(),
        };
        ...
        let rerun = queue(&conn, plan, RequestSource::Cli, "hash-1", &crate::catalog::scope::ProjectScope::default()).unwrap();
        match rerun {
    ...
    fn queued_job_can_be_claimed() {
        let mut conn = test_conn();
        let plan = seed_ready_plan(&conn);
        let id = match queue(&conn, plan, RequestSource::Cli, "h", &crate::catalog::scope::ProjectScope::default()).unwrap() {
            QueueOutcome::Created(id) => id,
            _ => unreachable!(),
        };
    ...
    fn worker_claims_oldest_job_deterministically() {
        ...
        let id_a = match queue(&conn, plan_a, RequestSource::Cli, "ha", &crate::catalog::scope::ProjectScope::default()).unwrap() {
            QueueOutcome::Created(id) => id,
            _ => unreachable!(),
        };
        let id_b = match queue(&conn, plan_b, RequestSource::Cli, "hb", &crate::catalog::scope::ProjectScope::default()).unwrap() {
            QueueOutcome::Created(id) => id,
            _ => unreachable!(),
        };
    ...
    fn failed_job_is_immutable_and_can_be_retried() {
        ...
        let new_id = retry(&conn, &id, RequestSource::Cli, "h", &crate::catalog::scope::ProjectScope::default()).unwrap();
        assert_ne!(new_id, id);
    ...
    fn cancelled_job_is_immutable_and_can_be_retried() {
        ...
        // Retry works.
        let new_id = retry(&conn, &id, RequestSource::LocalWeb, "h", &crate::catalog::scope::ProjectScope::default()).unwrap();
        assert_eq!(
    ...
    fn queue_event_is_transactional() {
        ...
        // initial event.
        let a = match queue(&conn, plan, RequestSource::Cli, "hash-a", &crate::catalog::scope::ProjectScope::default()).unwrap() {
            QueueOutcome::Created(id) => id,
            _ => unreachable!(),
        };
        let b = match queue(&conn, plan, RequestSource::Cli, "hash-b", &crate::catalog::scope::ProjectScope::default()).unwrap() {
            QueueOutcome::Created(id) => id,
            _ => unreachable!(),
        };
        assert_ne!(a, b);
    ...
    fn retry_rejects_incompatible_plan() {
        ...
        .unwrap();
        let err = retry(&conn, &id, RequestSource::Cli, "h", &crate::catalog::scope::ProjectScope::default()).unwrap_err();
    ...
    fn retry_uses_new_job_id_and_preserves_plan_revision() {
        ...
        .unwrap();
        let new_id = retry(&conn, &id, RequestSource::Cli, "h2", &crate::catalog::scope::ProjectScope::default()).unwrap();
        assert_ne!(new_id, id);
        let retried = get(&conn, &new_id).unwrap();
        assert_eq!(retried.plan_revision_id, plan);
    ...
    fn job_counts_by_execution_state() {
        ...
        queue(&conn, plan_a, RequestSource::Cli, "h", &crate::catalog::scope::ProjectScope::default()).unwrap();
        queue(&conn, plan_b, RequestSource::Cli, "h", &crate::catalog::scope::ProjectScope::default()).unwrap();
        queue(&conn, plan_c, RequestSource::Cli, "h", &crate::catalog::scope::ProjectScope::default()).unwrap();
