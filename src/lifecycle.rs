...
mod session32_tests {
    ...
    fn pilot_summary_uses_observer_scoped_language() {
        // The RouteViews pilot report specifically — the out directory
        // also contains RIPE RIS run reports; select by content
        // (collector route-views2), never by incident name.
        let report = std::fs::read_dir("case-studies/manlan-2019/pilot/out")
            ...
            .and_then(|mut it| {
                it.find_map(|e| {
                    ...
                    let txt = std::fs::read_to_string(p.join("report.txt")).ok()?;
                    if txt.to_lowercase().contains("route-views2") {
                        Some(txt)
                    } else {
                        None
                    }
