...
fn demo_manifest_matches_import() {
    ...
    // 4 event runs + 8 completed NORDUnet pilot runs (4 R&E + 4 RIS).
    assert_eq!(manifest["runs"], 12);
    assert_eq!(
        events, 21,
        "4 manifest events + 9 corpus events + 8 pilot events (INC0301970 is the second-network case)"
    );
    drop(conn);
}
